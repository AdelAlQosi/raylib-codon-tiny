...
If it needs a `Color` `Texture`, `Image*`, etc., write a thin wrapper in `bindings/bindings.c` that takes primitives or `void*` handles, rebuilds the raylib struct, and calls the real function. Then expose it in `__init__.py` and wrap it in `raylib.py` for a nice API.
